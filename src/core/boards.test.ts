...
describe("the board rack", () => {
  it("holds twenty boards: plain and nineteen to buy, with unique ids, names and themes", () => {
    expect(BOARDS.length).toBe(20);
    expect(new Set(BOARDS.map((b) => b.id)).size).toBe(20);
    expect(new Set(BOARDS.map((b) => b.name)).size).toBe(20);
    expect(new Set(BOARDS.map((b) => b.theme)).size).toBe(20);
  });
  ...
  it("keeps the plain board first and free, and prices the rest as a real save", () => {
    ...
    expect(BOARDS.slice(1).map((b) => b.cost)).toEqual([100, 250, 280, 300, 320, 330, 350, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1500, 1700, 1800, 1900]);
