 ...
 * finishes and you bail. Ollie ONTO a rail to grind, tap to hop off. Roll
 * up a HUMP and tap right at its lip to POP, with a trick thrown in free.
 * Every trick and grind before the board touches flat ground again joins
 * one CHAIN, and the chain banks its points times its length. Rails, boxes,
 ...
 *
 *  THE HUMP (0.22.0, Andy): "an inside out half pipe" from the ground. Up
 *  a bank, across a flat top whose length varies, down the mirror of the
 *  bank; sometimes one rail to grind along the middle of the top. */
export type ObstacleKind = "rail" | "box" | "gap" | "kicker" | "pipe" | "stairs" | "steps" | "hump";
...
/** How long a hump's bank is, on the way up and on the way down. The rest
 *  of its width is the flat top, so a wider hump has a longer top. */
export const HUMP_RAMP = 150;
/** How near the top of the bank a tap has to land to POP: about a third of
 *  a second of window at a walking line, a fifth once it is fast. */
export const HUMP_POP_X = 58;
/** The pop's launch. Air enough for the longest trick twice over, which is
 *  why the trick it throws in always lands. */
export const HUMP_POP_VY = 880;
/** How hard a bank leans on the line: it slows the climb by this much of
 *  the slope, and hurries the drop by the same. */
export const HUMP_GRIP = 0.34;
export interface Obstacle {
  ...
  x: number;
  w: number;
  /** Height of the top (rail, box, kicker lip, hump deck). Gaps have none. */
  h: number;
  /** For a rail that stands on something (a hump's flat top): how high its
   *  feet are. Art only; the model reads `h`, which is above the ground. */
  base?: number;
  /** Already collided with, or already launched from. */
  used: boolean;
}

export type Mode = "ground" | "air" | "grind" | "pipe" | "roll" | "slide" | "bail";
...
export interface Rider {
  ...
  trickT: number;
  /** The rail, or the handrail, under the trucks. */
  grindOn: Obstacle | null;
  grindT: number;
  /** The curve under the wheels: the pipe's bowl in "pipe", the hump's back
   *  in "roll". Null in every other mode. */
...
type Piece = { kind: ObstacleKind; dx: number; w: number; h: number; base?: number };

/** Where the bank tops out and the flat top begins: the lip to tap at. */
...
const spawn = (s: ParkState): void => {
  ...
  for (const p of pieces) {
    ...
    s.obstacles.push({ id: s.nextId++, kind: p.kind, x, w: p.w, h: p.h, used: false, ...(p.base !== undefined ? { base: p.base } : {}) });
...
export const surfaceY = (o: Obstacle, x: number): number => {
  ...
  // The hump is that bowl turned inside out, with a flat top let into the
  // middle of it: tangent to the ground at both feet, tangent to the deck
  // at both lips, so nothing on it is a corner.
  if (o.kind === "hump") {
    const u = x - o.x;
    const r = Math.min(HUMP_RAMP, o.w / 2);
    if (u <= 0 || u >= o.w) return 0;
    if (u < r) return o.h * Math.sin((Math.PI / 2) * (u / r)) ** 2;
    if (u > o.w - r) return o.h * Math.sin((Math.PI / 2) * ((o.w - u) / r)) ** 2;
    return o.h;
  }
...
/** How steep the surface is under a point: up the line is positive. */
export const surfaceSlope = (o: Obstacle, x: number): number => (surfaceY(o, x + 5) - surfaceY(o, x - 5)) / 10;

/**
 * What the ground is doing to the line right now. A bank slows the climb
 * and hurries the drop (Andy: "decelerate a bit up ... acceleration down
 * the parabola to ground level so it feels real"); everywhere else the
 * line runs at its own speed.
 */
export const terrainFactor = (s: ParkState): number => {
  const r = s.rider;
  if (r.mode !== "roll" || r.pipeOn === null) return 1;
  const k = 1 - HUMP_GRIP * surfaceSlope(r.pipeOn, riderX(s));
  return Math.max(0.68, Math.min(1.32, k));
};
...
export const press = (s: ParkState): void => {
  if (!s.running) return;
  const r = s.rider;
  if (r.mode === "ground" || r.mode === "grind" || r.mode === "slide" || r.mode === "roll") { r.holding = true; r.holdT = 0; }
};

const ollie = (s: ParkState, vy: number, ev: ParkEvent[], charge: number): void => {
  const r = s.rider;
  if (r.mode === "grind" || r.mode === "slide") endGrind(s, ev);
  if (r.mode === "roll") r.pipeOn = null;
  r.mode = "air";
  r.vy = vy;
...
export const release = (s: ParkState, swipe: Swipe | null, ev: ParkEvent[]): void => {
  ...
  // THE POP. A tap timed to the lip of a hump's bank launches bigger than
  // any ollie AND throws in a trick, which the air is long enough to land.
  if (r.mode === "roll" && swipe === null && r.pipeOn !== null && Math.abs(riderX(s) - humpLip(r.pipeOn)) <= HUMP_POP_X) {
    const t = PARK_TRICKS[Math.floor(rng(s) * PARK_TRICKS.length)]!;
    ollie(s, HUMP_POP_VY, ev, 1);
    ev.push({ kind: "pop", trick: t });
    startTrick(s, t, ev);
    return;
  }
  if (r.mode === "ground" || r.mode === "grind" || r.mode === "slide" || r.mode === "roll") {
...
export const update = (s: ParkState, dt: number): ParkEvent[] => {
  ...
  if (r.mode === "bail") {
    r.bailT += dt;
    if (r.bailT * 1000 >= BAIL_MS) {
      r.mode = "ground"; r.y = 0; r.vy = 0; r.bailWhy = null;
      // He gets up where he fell: on a hump's back, that is up on the hump.
      const on = s.obstacles.find((o) => o.kind === "hump" && over(s, o));
      if (on !== undefined) { r.mode = "roll"; r.pipeOn = on; r.y = surfaceY(on, riderX(s)); }
    }
    ...
    s.scroll += s.speed * terrainFactor(s) * dt;
  }
  ...
  if (r.mode === "air") {
    ...
    // A hump's back catches anything coming down onto it: it is ground that
    // happens to be curved, so the chain banks here as it would below.
    for (const o of s.obstacles) {
      if (o.kind !== "hump" || r.vy > 0 || !over(s, o)) continue;
      const sy = surfaceY(o, riderX(s));
      if (r.y > sy + 10) continue;
      if (!settleTrick(s, ev)) return ev;
      r.mode = "roll"; r.pipeOn = o; r.y = sy; r.vy = 0;
      ev.push({ kind: "land" });
      bank(s, ev);
      return ev;
    }
  ...
  if (r.mode === "roll") {
    const o = r.pipeOn!;
    const x = riderX(s);
    if (x >= o.x + o.w) { r.pipeOn = null; r.mode = "ground"; r.y = 0; return ev; }
    // A rail let into the flat top is a wall like any other: ollie onto it.
    for (const q of s.obstacles) {
      if (q.kind !== "rail" || q.used) continue;
      if (x + RIDER_W / 2 > q.x && x - RIDER_W / 2 < q.x + 18 && r.y < q.h - 18) { q.used = true; bail(s, "rail", ev); return ev; }
    }
    r.y = surfaceY(o, x);
    return ev;
  }
  ...
  // On the ground: humps are rolled up, kickers launch, gaps swallow, rails
  // and boxes stop you.
  for (const o of s.obstacles) {
    const x = riderX(s);
    if (o.kind === "hump" && over(s, o)) {
      r.mode = "roll"; r.pipeOn = o; r.y = surfaceY(o, x); r.vy = 0;
      return ev;
    }
