...
#
# Usage: ./run-gauntlet.sh [--quick [sense...]] [--no-deploy]
#
# Senses: probe-loop answer-eye human-eye tap-audit legible-check offline-check
#
# ONE CHANNEL. Production only, like the mortgage analyser and unlike the game
# repos. There is no staging URL to catch what this misses, so the FULL
# gauntlet is the law before every version stamp.
set -o pipefail
...
wants() { [[ $QUICK -eq 0 ]] && return 0; [[ " ${WANT[*]} " == *" $1 "* ]]; }
...
echo "PORTS: $PROJECT holds $BASE_PORT-$((BASE_PORT + 49))"
...
wants probe-loop    && launch probe-loop    "$B node tools/probe-loop.mjs"
...
stage "stage 5/5: deploy + converge + live smoke"
...
# production address untouched while reporting a perfect upload.
npx wrangler pages deploy dist --project-name=$PROJECT --branch=main --commit-dirty=true 2>&1 | tail -4
